document.addEventListener('DOMContentLoaded', () => {
  ...
  // ===============================================
  // 🌙 Lógica de Tema (Dark Mode) - Não alterada
  // ===============================================
  ...
  const showCard = (cardToShow) => {
    [loginCard, diagnosisCard, resultsCard].forEach(card => {
      if (card === cardToShow) {
        ...
        // Rola para o topo do card ao mostrar
    ...
    currentSection.questions.forEach(question => {
      ...
      const createAnswerButton = (label, value) => {
        ...
        button.addEventListener('click', () => {
          ...
          button.classList.add('selected');
          // Não renderiza plano de ação para 'no' conforme a nova regra
    ...
    questionnaireData.sections.forEach(section => {
      ...
      section.questions.forEach(question => {
        ...
        if (answer === 'yes') {
          ...
          sectionAnsweredCount++;
          // A lógica de adicionar plano de ação como aviso importante foi removida.
        } else if (answer === 'na') {
            sectionAnsweredCount++;
  ...
  signInForm.addEventListener('submit', (event) => {
    ...
    // Validação do número de telefone (já está com pattern no HTML, mas podemos adicionar um feedback JS)
    const phoneInput = document.getElementById('phone');
    if (!phoneInput.checkValidity()) {
        alert('Por favor, insira um número de telefone válido no formato: +55 (DDD) 9XXXX-XXXX');
        return;
    }
